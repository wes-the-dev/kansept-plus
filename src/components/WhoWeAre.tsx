...
const FALLBACK_TEAM = [
  { name: "Name Placeholder", role: "Lead Architect", photo: null },
  { name: "Name Placeholder", role: "Senior Interior Designer", photo: null },
  { name: "Name Placeholder", role: "Creative Director", photo: null },
  { name: "Name Placeholder", role: "Construction Manager", photo: null },
];
...
export const WhoWeAre = ({ sanityTeam, settings }: WhoWeAreProps) => {
  ...
  const topImage = resolveImageUrl(wwa?.topImage) || "/images/who-we-are-top.png";
  ...
  const aboutPhoto = resolveImageUrl(wwa?.aboutPhoto);
  ...
  const team =
    sanityTeam && sanityTeam.length > 0
      ? sanityTeam.map((m) => ({
          name: m.name,
          role: m.role || "",
          photo: resolveImageUrl(m.photo, 600),
        }))
      : FALLBACK_TEAM;

  const processSteps = wwa?.processSteps;

  const contactImg1 = resolveImageUrl(settings?.contact?.contactImage1) || contactImg1Default;
  const contactImg2 = resolveImageUrl(settings?.contact?.contactImage2) || contactImg2Default;
  ...
  return (
    <div className="bg-[#FFF3EB] text-[#1a3749] overflow-hidden pt-[100px]">
      ...
      <section className="bg-[#1A3749] text-[#FFF3EB] px-6 md:px-[60px] py-20 md:py-32 grid grid-cols-1 md:grid-cols-2 gap-10 md:gap-20 items-center">
        ...
        >
          {aboutPhoto ? (
            <img src={aboutPhoto} alt="About Kansept" className="w-full h-full object-cover" />
          ) : (
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-8 text-[12px] uppercase tracking-[2px] text-[#b5754d]">
            <div>
              ...
              <p className="opacity-70">{leadDesignerName}</p>
            </div>
            <div>
              <p className="mb-2 text-white">Focus</p>
              <p className="opacity-70">{leadDesignerFocus}</p>
            </div>
          </div>
        </motion.div>
      </section>
      ...
      <section className="px-6 md:px-[60px] py-20 md:py-32 relative bg-[#FFF3EB]">
        ...
        <div className="relative">
          ...
          {(processSteps && processSteps.length > 0 ? processSteps.map((s) => ({ ...s, image: s.image ? resolveImageUrl(s.image) || "" : "" })) : FALLBACK_STEPS).map((step, index) => {
            const isEven = index % 2 === 1; // even-indexed steps flip layout
            const isLast = index === (processSteps?.length ?? FALLBACK_STEPS.length) - 1;
            ...
            const textEl = (
              <motion.div
                ...
                className={isEven ? "md:text-right pr-0 md:pr-16 order-1 flex flex-col items-start md:items-end" : "pl-0 md:pl-16 order-1 md:order-2"}
              >
                <div className="text-[40px] font-light text-[#b5754d] mb-4">{step.number}</div>
                <h3 className={`text-[14px] font-bold uppercase tracking-[2px] mb-2 bg-[#B5754D] inline-block px-2 py-1 transform ${isEven ? "rotate-1" : "-rotate-1"} text-white`}>
                  {step.title}
                </h3>
            ...
            <form className="space-y-8" onSubmit={handleSubmit}>
                <div className="grid grid-cols-2 gap-8">
                  <div className="border-b border-[#1a3749]/20 pb-2">
                    ...
                    <input
                      ...
                      className="w-full bg-transparent border-none focus:ring-0 p-0 text-[#1a3749] placeholder-[#1a3749]/30"
                    />
                  </div>
                  <div className="border-b border-[#1a3749]/20 pb-2">
                ...
                <div className="border-b border-[#1a3749]/20 pb-2">
                  ...
                  <select
                    ...
                    className="w-full bg-transparent border-none focus:ring-0 p-0 text-[#1a3749]"
                  >
                ...
                <div className="border-b border-[#1a3749]/20 pb-2">
                  ...
                  <textarea
                    ...
                    className="w-full bg-transparent border-none focus:ring-0 p-0 text-[#1a3749] placeholder-[#1a3749]/30 resize-none"
                  />
                </div>
                ...
                <div className="flex items-center gap-3 pt-4">
                  <input
                    ...
                    className="rounded-none border-[#1a3749]/30 text-[#b5754d] focus:ring-[#b5754d]"
                  />
                ...
                <button
                  ...
                  className="bg-[#3E2723] text-white px-10 py-4 text-[11px] uppercase tracking-[3px] hover:bg-[#1a3749] transition-colors mt-4 disabled:opacity-50 disabled:cursor-not-allowed"
                >
