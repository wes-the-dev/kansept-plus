...
export const BlogPost = ({ id, sanityPost }: BlogPostProps) => {
  ...
  const coverMedia = sanityPost ? resolveMediaAsset(sanityPost.mainImage, 1200) : null;
  ...
  const post = sanityPost
    ? {
        ...
        image: coverMedia?.url || "/images/blog-1.jpg",
        isVideo: coverMedia?.isVideo ?? false,
        portableContent: sanityPost.content || null,
        content: null as string[] | null,
      }
    : fallbackPost
    ? { ...fallbackPost, portableContent: null, isVideo: false }
    : null;
    ...
    <div className="bg-[#FFF3EB] text-[#1a3749] min-h-screen pt-[100px]">
      <article className="max-w-4xl mx-auto px-6 md:px-10 py-10 md:py-20">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }} className="mb-12">
          ...
          <h1 className="text-3xl md:text-5xl lg:text-6xl font-light leading-tight mb-8 text-[#1a3749]">{post.title}</h1>
          ...
          <div className="flex items-center justify-between border-t border-b border-[#1a3749]/10 py-6">
            <div className="flex items-center gap-6 text-[13px] text-[#1a3749]/70 font-light">
              <div className="flex items-center gap-2"><User size={16} className="text-[#b5754d]" /><span>{post.author}</span></div>
              <div className="flex items-center gap-2"><Clock size={16} className="text-[#b5754d]" /><span>{post.readTime}</span></div>
            </div>
        ...
        >
          {post.isVideo ? (
            <video src={post.image} controls autoPlay playsInline className="w-full h-full object-cover" />
          ) : (
        ...
        >
          <div className="prose prose-lg prose-headings:font-light prose-headings:text-[#1a3749] prose-p:text-[#1a3749]/80 prose-p:font-light prose-p:leading-loose">
            {post.portableContent ? (
              ...
              post.content?.map((paragraph, index) => (
                <p key={index} className="mb-8 text-[18px]">{paragraph}</p>
              ))
            )}
          </div>
          ...
          <div className="mt-16 pt-8 border-t border-[#1a3749]/10 flex flex-col md:flex-row items-center justify-between gap-6">
            ...
            <div className="flex items-center gap-4">
              <button className="w-10 h-10 rounded-full border border-[#1a3749]/20 flex items-center justify-center text-[#1a3749]/60 hover:bg-[#1a3749] hover:text-white transition-all"><Facebook size={16} /></button>
              <button className="w-10 h-10 rounded-full border border-[#1a3749]/20 flex items-center justify-center text-[#1a3749]/60 hover:bg-[#1a3749] hover:text-white transition-all"><Twitter size={16} /></button>
              <button className="w-10 h-10 rounded-full border border-[#1a3749]/20 flex items-center justify-center text-[#1a3749]/60 hover:bg-[#1a3749] hover:text-white transition-all"><Linkedin size={16} /></button>
            </div>
          </div>
        </motion.div>
